...
public class ShipRotationManager_ShipSelection : MonoBehaviour
{
    ...
    private const float minSinInc = 0.015f;
    private const float maxSinInc = 0.025f;
    ...
    private Vector3 joinAllEulerAngles()
    {
        return new Vector3(Mathf.Sin(sinVal_x) * maxTumbleAngle, 
                            Mathf.Sin(sinVal_y) * maxTumbleAngle,
                            Mathf.Sin(sinVal_z) * maxTumbleAngle);
    }
